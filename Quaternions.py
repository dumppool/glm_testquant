...
class Quaternions:
    ...
    def __init__(self, qs):
        ...
        raise TypeError(
            'Quaternions must be constructed from iterable, numpy array, or Quaternions, not %s' % type(qs))
    ...
    def _broadcast(cls, sqs, oqs, scalar=False):
        ...
        if len(ss) != len(os):
            raise TypeError('Quaternions cannot broadcast together shapes %s and %s' % (
                sqs.shape, oqs.shape))
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, np.ndarray) and other.shape[-1] == 3:
            vs = Quaternions(np.concatenate(
                [np.zeros(other.shape[:-1] + (1,)), other], axis=-1))
    ...
    def average(self):
        ...
        if len(self.shape) == 1:
            ...
            system = ut.matrix_multiply(
                self.qs[:, :, np.newaxis], self.qs[:, np.newaxis, :]).sum(axis=0)
            w, v = np.linalg.eigh(system)
            qiT_dot_qref = (self.qs[:, :, np.newaxis]
                            * v[np.newaxis, :, :]).sum(axis=1)
            return Quaternions(v[:, np.argmin((1. - qiT_dot_qref**2).sum(axis=0))])

        else:

            raise NotImplementedError(
                'Cannot average multi-dimensionsal Quaternions')
    ...
    def id(cls, n):
        ...
        raise TypeError(
            'Cannot Construct Quaternion from %s type' % str(type(n)))
    ...
    def from_angle_axis(cls, angles, axis):
        axis = axis / (np.sqrt(np.sum(axis**2, axis=-1)) +
                       1e-10)[..., np.newaxis]
    ...
    @classmethod
    def from_transforms_toUE(cls, ts, adj_axis='z', adj_degree=0, gbl=True):
        R1 = ts[..., 0:3, 0:3]
        R12 = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]])
        R21 = np.transpose(R12)
        R2 = np.matmul(R12, np.matmul(R1, R21))
        # R2 = ts[..., 0:3, 0:3]

        adj_euler = [0., 0., 0.]
        adj_euler['xyz'.index(adj_axis)] = adj_degree
        Radj = R.from_euler('XYZ', adj_euler, degrees=True).as_dcm()
        if gbl:
            R2[:, 1:, ...] = np.matmul(Radj, R2[:, 1:, :, :])
        else:
            R2[:, 1, ...] = np.matmul(Radj, R2[:, 1, :, :])

        R2 = R2.reshape(-1, 3, 3)
        quat = R.from_dcm(R2).as_quat().reshape(*(ts.shape[:-2] + (4,)))
        quat = quat[..., [3, 0, 1, 2]]  # CHANGE to w, x, y, z
        return cls(quat)

    @classmethod
    def from_transforms_toDM(cls, ts, adj_axis='z', adj_degree=0, gbl=True):
        Rot = ts[..., 0:3, 0:3]
        adj_euler = [0., 0., 0.]
        adj_euler['xyz'.index(adj_axis)] = adj_degree
        Radj = R.from_euler('XYZ', adj_euler, degrees=True).as_dcm()
        if gbl:
            Rot[:, 1:, ...] = np.matmul(Radj, Rot[:, 1:, :, :])
        else:
            Rot[:, 1, ...] = np.matmul(Radj, Rot[:, 1, :, :])

        Rot = Rot.reshape(-1, 3, 3)
        quat = R.from_dcm(Rot).as_quat().reshape(*(ts.shape[:-2] + (4,)))
        quat = quat[..., [3, 0, 1, 2]]
        return cls(quat)

    def quat2fwdup(self, forward='x', up='z'):
        v_qs = self.qs.reshape(-1, 4)
        # quat from (w, x, y, z) to (x, y, z, w)
        v_qs = v_qs[..., [1, 2, 3, 0]]
        r = R.from_quat(v_qs)
        dcm = r.as_dcm()
        dcm = dcm.reshape(*(self.qs.shape[:-1] + (3, 3)))
        return dcm[..., 'xyz'.index(forward)], dcm[..., 'xyz'.index(up)]
