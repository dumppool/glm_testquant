...
double cDeepMimicCore::GetKinTime() const
{
	if (EnableDraw())
	{
		std::shared_ptr<cDrawSceneImitate> mDrawSceneImitate;
		mDrawSceneImitate = std::dynamic_pointer_cast<cDrawSceneImitate>(mScene);
		return mDrawSceneImitate->GetKinTime();

	} else 
	{
		std::shared_ptr<cSceneImitate> mSceneImitate;
		mSceneImitate = std::dynamic_pointer_cast<cSceneImitate>(mScene);
		return mSceneImitate->GetKinTime();
	}
...
void cDeepMimicCore::PrintArgs() const
{
	mArgParser->PrintArgs();
}

void cDeepMimicCore::SetArgs(const std::map<std::string, std::vector<std::string>>& args)
{
	mArgParser->SetArgs(args);
	mArgParser->ParseInt("num_update_substeps", mNumUpdateSubsteps);
}
