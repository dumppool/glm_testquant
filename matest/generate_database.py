...
from phygamenn.utils.Pivots import Pivots
from phygamenn.utils.Learning import RBF
...
file_list = [f for f in sorted(list(os.listdir(src_dbroot))) if os.path.isfile(
    os.path.join(src_dbroot, f)) and f.endswith('.bvh')]
...
def process_data(anim, phase, gait, type='flat'):
    ...
    for i in range(window, len(anim) - window - 1, 1):
        ...
        rootdirs_next = root_rotation[i + 1:i +
                                      2, 0] * forward[i + 1:i + window + 1:10]
...
for file in file_list:
    ...
    print('Processing Clip %s' % file)

    """ Data Types """

    if 'LocomotionFlat12_000' in file:
        type = 'jumpy'
    elif 'NewCaptures01_000' in file:
        type = 'flat'
    elif 'NewCaptures02_000' in file:
        type = 'flat'
    elif 'NewCaptures03_000' in file:
        type = 'jumpy'
    elif 'NewCaptures03_001' in file:
        type = 'jumpy'
    elif 'NewCaptures03_002' in file:
        type = 'jumpy'
    elif 'NewCaptures04_000' in file:
        type = 'jumpy'
    elif 'WalkingUpSteps06_000' in file:
        type = 'beam'
    elif 'WalkingUpSteps09_000' in file:
        type = 'flat'
    elif 'WalkingUpSteps10_000' in file:
        type = 'flat'
    elif 'WalkingUpSteps11_000' in file:
        type = 'flat'
    elif 'Flat' in file:
        type = 'flat'
    else:
    ...
    anim, names, _ = BVH.load(os.path.join('data/dm/deepm', file))
    ...
    phase = np.loadtxt(os.path.join(
        src_dbroot, file.replace('.bvh', '.phase')))[::2]
    gait = np.loadtxt(os.path.join(
        src_dbroot, file.replace('.bvh', '.gait')))[::2]
    ...
    with open(os.path.join(src_dbroot, file.replace('.bvh', '_footsteps.txt')), 'r') as f:
    ...
    for li in range(len(footsteps) - 1):
        ...
        for h, hmean in zip(H, Hmean):
            ...
            xo_s, xo_e = ((window * 2) // 10) * 10 + \
                1, ((window * 2) // 10) * 10 + njoints * 3 + 1
            yo_s, yo_e = 8 + (window // 10) * 4 + 1, 8 + \
                (window // 10) * 4 + njoints * 3 + 1
