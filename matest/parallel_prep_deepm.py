...
if __name__ == '__main__':
    ...
    rtgt_dbroot = "./data/dm/rtgt/"
    res_dbroot = "./data/dm/deepm/"

    if not os.path.isdir(rtgt_dbroot):
        os.mkdir(rtgt_dbroot)
    if not os.path.isdir(res_dbroot):
        os.mkdir(res_dbroot)

    orig_anim, orig_names, orig_frames = BVH.load(orig_dbroot + 'rest.bvh')

    # gps = Animation.positions_global(orig_anim)
    # gp0 = gps[0, :, :]
    # h_head = gp0[2, 1]
    # h_foot = (gp0[11, 1] + gp0[15, 1]) / 2
    # h = h_head - h_foot
    # print(h)

    src_anim, src_names, src_frames = BVH.load(src_dbroot + 'rest.bvh')
    # gps = Animation.positions_global(src_anim)
    # gp0 = gps[0, :, :]
    # h_head = gp0[15, 1]
    # h_foot = (gp0[4, 1] + gp0[9, 1]) / 2
    # h = h_head - h_foot
    # print(h)

    orig_anim.offsets = orig_anim.offsets * 2
    BVH.save(orig_dbroot + 'rest_scaled.bvh', orig_anim, orig_names)

    src_bvh_files = [src_dbroot + f for f in file_list]
    rtgt_bvh_files = [rtgt_dbroot + f for f in file_list]
    res_bvh_files = [res_dbroot + f for f in file_list]

    po = Pool(pnum)
    for i, bvh_file in enumerate(src_bvh_files):
        po.apply_async(
            retarget, (orig_dbroot + 'rest_scaled.bvh', rtgt_bvh_files[i], bvh_file))
    print("---Retarget---")
    po.close()
    po.join()

    po = Pool(pnum)
    for i, bvh_file in enumerate(res_bvh_files):
        po.apply_async(
            simplify_and_scale, (rtgt_bvh_files[i], bvh_file, 1))
    print("---Simplify and Scale---")
    po.close()
    po.join()
    ...
    for i, bvh_file in enumerate(res_bvh_files):
        po.apply_async(
            bvh2json, (bvh_file, bvh_file.replace(".bvh", ".txt")))
    print("---BVH to JSON---")
    po.close()
    po.join()
