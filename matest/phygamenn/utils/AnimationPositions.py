...
import phygamenn.utils.AnimationStructure as AnimationStructure
...
def load_from_maya(root, start, end):
    ...
    def rig_joints_list(s, js):
        for c in s.getChildren():
            if 'Geo' in c.name():
                continue
            if isinstance(c, pm.nodetypes.Joint):
                js = rig_joints_list(c, js)
                continue
            if isinstance(c, pm.nodetypes.Transform):
                js = rig_joints_list(c, js)
                continue
        return [s] + js
    ...
    for i in range(start, end):
        ...
        for j in joints:
            positions[i - start,
                      names.index(j.name())] = j.getTranslation(space='world')
...
def redirect(positions, joint0, joint1, forward='z'):
    ...
    direction = (positions[:, joint0] - positions[:, joint1]
                 ).mean(axis=0)[np.newaxis, np.newaxis]
