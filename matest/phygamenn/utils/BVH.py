...
from phygamenn.utils.Animation import Animation
...
def func_root_rot(root_rot_euler, angs_euler):
    angs_mat = R.from_euler('XYZ', angs_euler, degrees=True).as_dcm()
    # print(angs_mat)
    root_rot_mat = R.from_euler('ZYX', root_rot_euler, degrees=True).as_dcm()
    root_rot_mat2 = np.matmul(angs_mat, root_rot_mat)
    root_rot_euler2 = R.from_dcm(root_rot_mat2).as_euler('ZYX', degrees=True)
    return root_rot_euler2


def load_and_laydown(filename, start=None, end=None, order=None, world=False):
    """
    Reads a BVH file and constructs an animation
    ...
    for line in f:
        ...
        if dmatch:
            ...
            if channels == 3:
                # assign the first joint [0] the root position,
                # the position of remaining joints [1:end] are assigned as offsets
                positions[fi, 0:1] = data_block[0:3]
                rotations[fi, :] = data_block[3:].reshape(N, 3)
            elif channels == 6:
                data_block = data_block.reshape(N, 6)
    ...
    f.close()
    # ADDED
    root_rot_euler = rotations[:, 0, :]
    # print('ZYX:', root_rot_euler[0,:])
    root_rot_euler2 = func_root_rot(root_rot_euler, [90, 0, 0])
    root_rot_euler3 = func_root_rot(root_rot_euler2, [0, 0, 90])
    # print('ZYX:', root_rot_euler3[0, :])
    rotations[:, 0, :] = root_rot_euler3
    positions[:, 0, :] = positions[:, 0, [2, 0, 1]]
    # positions[:, 0, 1] = -positions[:, 0, 1]
    # ADDED
    # rotations_euler_gl = rotations
...
def save(filename, anim, names=None, frametime=1.0 / 24.0, order='zyx', positions=False, orients=True):
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(anim.shape[1]):
            if anim.parents[i] == 0:
                t = save_joint(f, anim, names, t, i, save_order,
...
def save_joint(f, anim, names, t, i, save_order, order='zyx', positions=False):
    ...
    for j in range(anim.shape[1]):
        if anim.parents[j] == i:
            t = save_joint(f, anim, names, t, j, save_order,
                           order=order, positions=positions)
            end_site = False
