    ...
    for j in xrange(anim.shape[1]):
        ...
        for c0 in children[j]:
            ...
            for c1 in children[j]:
                ...
                dists = np.sum(
                    (points[:, c0] - points[:, c1])**2.0, axis=1)**0.5
                constraints.append(
                    (c0, c1, masses[:, c0], masses[:, c1], dists))
...
def distances(anim):
    ...
    def find_distance(distances, generated, prev, i, j):
        ...
        par_dists = [(joint_lengths[j], find_distance(
            distances, generated, j, i, p)) for p in joint_parents[j] if p != prev]
        out_dists = [(joint_lengths[c], find_distance(
            distances, generated, j, i, c)) for c in joint_children[j] if c != prev]
